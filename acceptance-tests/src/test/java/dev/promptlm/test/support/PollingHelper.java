...
/**
 * Shared polling primitive for the acceptance-test {@code HappyPathUserJourneyTest}.
 *
 * <p>The acceptance suite has several places that need to poll an out-of-process
 * resource (Gitea repository state, Artifactory deployments, ...) until a
 * condition becomes true, with a bounded timeout and diagnostic output on
 * failure. Before this helper, each site re-implemented the
 * {@link org.awaitility.Awaitility} + {@link java.util.concurrent.atomic.AtomicReference}
 * scaffolding with subtly different timeout-message shapes.
 *
 ...
 * <h2>Example</h2>
 * <pre>{@code
 * java.util.concurrent.atomic.AtomicReference<String> lastSeen = new java.util.concurrent.atomic.AtomicReference<>();
 *
 ...
 * own timeout message exactly as before.
 */
public final class PollingHelper {
    ...
    public static <T> Result<T> pollUntil(
        ...
        java.util.concurrent.atomic.AtomicReference<Throwable> lastError = new java.util.concurrent.atomic.AtomicReference<>();
